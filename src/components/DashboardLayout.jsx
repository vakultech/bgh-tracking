...
export default function DashboardLayout({ children, role }) {
  ...
  return (
    <div className="dashboard-root">
          ...
          {currentMenu.map((item) => (
            <div key={item.path} className="nav-group">
              <button 
                ...
                onClick={() => item.sub ? null : navigate(item.path)}
              >
                <item.icon size={22} />
      ...
      <style jsx>{`
        .dashboard-root { display: flex; height: 100vh; background: #f8fafc; overflow: hidden; }
        
        .sidebar { background: #1e3a8a; color: white; display: flex; flex-direction: column; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 4px 0 20px rgba(0,0,0,0.1); z-index: 100; }
        ...
        .sidebar-nav { flex: 1; padding: 1.25rem; display: flex; flex-direction: column; gap: 0.5rem; }
        ...
        .menu-toggle { background: var(--bg); border: 1px solid var(--border); border-radius: 8px; padding: 0.5rem; transition: all 0.2s; cursor: pointer; }
        ...
        .user-avatar { width: 40px; height: 40px; background: linear-gradient(135deg, var(--primary), var(--primary-light)); color: white; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: 700; }
